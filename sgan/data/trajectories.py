...
def seq_collate(data):
    ...
    pred_traj = torch.tensor([])#torch.cat(pred_seq_list, dim=0).permute(2, 0, 1)
    obs_traj_rel = torch.cat(obs_seq_rel_list, dim=0).permute(2, 0, 1)
    pred_traj_rel = torch.tensor([])#torch.cat(pred_seq_rel_list, dim=0).permute(2, 0, 1)
    non_linear_ped = torch.tensor([])#torch.cat(non_linear_ped_list)
    loss_mask = torch.tensor([])#torch.cat(loss_mask_list, dim=0)
...
class TrajectoryDataset(Dataset):
    ...
    def __init__(
        self, data, obs_len=8, pred_len=12, skip=1, threshold=0.002,
        min_ped=1, delim='\t'
    ):
        """
        Args:
...
#            curr_ped_seq = curr_ped_seq
        ...
        self.pred_traj_rel = None
        self.loss_mask = None 
        self.non_linear_ped = None
